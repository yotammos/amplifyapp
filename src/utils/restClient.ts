...
export const buildRequest = (path: string, isLocal: boolean, subPath?: string) => {
  const completePath: string = subPath ? `${path}/${subPath}` : path
  return isLocal
    ? { path: completePath } as DetailedRequest
    : { path: completePath, host: 'mossbeanstalk-env.eba-uetagxqr.us-east-2.elasticbeanstalk.com', port: 80 } as DetailedRequest
}
